...
import type { FreezeWeapon } from '../../../components/weapon/freeze-weapon';
import type { CircleCollider } from '../../../../types/collider';
import { findTeam } from '../utils/find-team';
import { FREEZE_EFFECT_ID } from '../../../../consts/templates';

import type { Attack } from './attack';

const EXPLOSION_LIFETIME = 100;

export class FreezeAttack implements Attack {
  private actor: Actor;
  ...
  constructor(actor: Actor, spawner: ActorSpawner, scene: Scene, angle: number) {
    ...
    const {
      ...
      projectileModel,
      projectileRadius,
    } = this.weapon.properties as FreezeWeapon;
  ...
  private explode(): void {
    ...
    const { explosionModel, explosionRadius } = this.weapon.properties as FreezeWeapon;
  ...
  private handleExplosionCollisionEnter = (event: CollisionEnterEvent): void => {
    ...
    const { damage, slowFactor } = this.weapon.properties as FreezeWeapon;
    ...
    target.dispatchEvent(EventType.AddEffect, { id: FREEZE_EFFECT_ID, options: { slowFactor } });
  };
